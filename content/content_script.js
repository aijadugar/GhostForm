...
function getCustomFieldPersonaValue(field, persona) {
  ...
  for (const customField of customFields) {
    ...
    const customKey = normalize(customField.key);
    const customLabel = normalize(customField.label);
    const matchesCustomField = searchableValues.some((value) => {
      return (
        value &&
        ((customKey && value.includes(customKey)) ||
          (customLabel && value.includes(customLabel)))
      );
    });
...
function getPersonaValue(field, persona) {
  ...
  const customFieldValue = getCustomFieldPersonaValue(field, persona);

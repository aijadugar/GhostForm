(function () {
  ...
  const USER_PROFILE_KEY = "gf_user_profile";
  const FILL_MODE_KEY = "gf_fill_mode";
  ...
  globalThis.Vault = {
    ...
    async getUserProfile() {
      const result = await storageGet(USER_PROFILE_KEY);
      return result[USER_PROFILE_KEY] || null;
    },

    async saveUserProfile(profile) {
      await storageSet({ [USER_PROFILE_KEY]: profile });
    },

    async getFillMode() {
      const result = await storageGet(FILL_MODE_KEY);
      return result[FILL_MODE_KEY] === "profile" ? "profile" : "ai";
    },

    async setFillMode(mode) {
      const fillMode = mode === "profile" ? "profile" : "ai";
      await storageSet({ [FILL_MODE_KEY]: fillMode });
    }
  };
})();
